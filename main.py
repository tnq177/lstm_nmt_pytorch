...
from model import Transformer, LstmNMT
...
def get_parser():
    ...
    parser.add_argument('--mode', choices=['train', 'translate', 'probs'], default='train')
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        if args.model_file and exists(args.model_file):
            logger.info('IMPORTANT!!!!! RELOAD CHECKPOINT {}'.format(args.model_file))
        ...
        files_langs = args.files_langs
        for fl in files_langs:
            input_file, ref_file, src_lang, tgt_lang = fl.split(',')
            controller.calc_probs(input_file, ref_file, src_lang, tgt_lang)
    else:
