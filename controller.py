...
class Controller(object):
    ...
    def calc_probs(self, src_file, ref_file, src_lang, tgt_lang, batch_size=4096):
        self.model.eval()
        src_lang_idx = self.data_manager.lang_vocab[src_lang]
        tgt_lang_idx = self.data_manager.lang_vocab[tgt_lang]
        logit_mask = self.data_manager.logit_masks[tgt_lang]
        # calculating probs
        probs_file = ref_file + '.probs'
        open(probs_file, 'w').close()
        with open(probs_file, 'w') as fout, open(src_file, 'r') as src_fin, open(ref_file, 'r') as tgt_fin:
            for src_line, tgt_line in zip(src_fin, tgt_fin):
                src_ids = src_line.strip().split()
                tgt_ids = tgt_line.strip().split()
                if not src_ids or not tgt_ids:
                    raise ValueError('Empty line detected. Please make sure there is no empty line.')

                src_ids = [self.data_manager.vocab.get(w, ac.UNK_ID) for w in src_ids]
                tgt_ids = [self.data_manager.vocab.get(w, ac.UNK_ID) for w in tgt_ids]
                tgt_len = len(tgt_ids) + 1
                src_batch = np.array(src_ids + [ac.EOS_ID]).reshape(1, -1)
                tgt_batch = np.array([ac.BOS_ID] + tgt_ids).reshape(1, -1)
                target_batch = np.array(tgt_ids + [ac.EOS_ID]).reshape(1, -1)
                src_batch = torch.from_numpy(src_batch).to(self.device)
                tgt_batch = torch.from_numpy(tgt_batch).to(self.device)
                target_batch = torch.from_numpy(target_batch).to(self.device)
                ret = self.model(src_batch, tgt_batch, target_batch, src_lang_idx, tgt_lang_idx, logit_mask)
                logits = ret['logits'].reshape(tgt_len, -1)
                probs = torch.nn.functional.softmax(logits, dim=-1)
                probs = probs.gather(dim=-1, index=target_batch.reshape(tgt_len, 1))
                probs = probs.detach().cpu().numpy().reshape(-1)

                tgt_words = tgt_line.strip().split() + [ac._EOS]
                output = ["{}<<<SPLIT>>>{}".format(tgt_words[i], probs[i]) for i in range(tgt_len)]
                output = ' '.join(output) + '\n'
                fout.write(output)
