    ...
    def load_data(self):
        ...
        for pair in self.pairs:
            ...
            for mode in [ac.TRAIN, ac.DEV]:
                ...
                self.data[pair][mode] = NMTDataset(src, tgt, self.args, mode)
    ...
    def get_translate_batches(self, src_file, batch_size=4096):
        ...
        with open(src_file, 'r') as fin:
            for line in fin:
                ...
                if toks:
                    # make sure to append EOS at the end of source sentence
...
class NMTDataset(object):
    def __init__(self, src, tgt, args, mode):
        ...
        self.data_dup = args.data_dup
        self.data_dup_rand = args.data_dup_rand
        self.regen_batches = args.regen_batches
        self.length_limit = args.length_limit

        if self.mode == ac.TRAIN and self.data_dup:
            self.args.logger.info('Concate examples to generate examples')
            if self.data_dup_rand:
                self.args.logger.info('Shuffling before concat')
            ...
            for i in range(1, src.shape[0]):
                new_src = src[i - 1] + src[i]
                new_tgt = tgt[i - 1] + tgt[i]
                if 0 < len(new_src) <= self.length_limit and 0 < len(new_tgt) <= self.length_limit:
                    extra_src.append(new_src)
                    extra_tgt.append(new_tgt)

            src = np.concatenate((src, np.array(extra_src)), axis=0)
            tgt = np.concatenate((tgt, np.array(extra_tgt)), axis=0)
            self.args.logger.info('==> {} training examples'.format(src.shape[0]))
    ...
    def gen_batches(self):
        # reset batches & indices
        self.batches = []
        # randomize data
        idxs = list(range(self.src.shape[0]))
        np.random.shuffle(idxs)
        self.src = self.src[idxs]
        self.tgt = self.tgt[idxs]
        CHUNK = 10000
        for i in range(0, self.src.shape[0], CHUNK):
            src = np.copy(self.src[i:i + CHUNK])
            tgt = np.copy(self.tgt[i:i + CHUNK])
            ...
            while s_idx < src.shape[0]:
                ...
                while e_idx < src.shape[0]:
                    ...
                    num_toks = (e_idx - s_idx + 1) * (max(max_src_in_batch, max_tgt_in_batch) + 1) # +1 for BOS/EOS
    ...
    def prepare_one_batch(self, src, tgt, src_lens, tgt_lens):
        ...
        # + 1 for BOS/EOS
        src_batch = np.zeros([num_sents, max_src_len + 1], dtype=np.int32)
        tgt_batch = np.zeros([num_sents, max_tgt_len + 1], dtype=np.int32)
        target_batch = np.zeros([num_sents, max_tgt_len + 1], dtype=np.int32)

        for i in range(num_sents):
            src_batch[i] = src[i] + [ac.EOS_ID] + (max_src_len - src_lens[i]) * [ac.PAD_ID]
            tgt_batch[i] = [ac.BOS_ID] + tgt[i] + (max_tgt_len - tgt_lens[i]) * [ac.PAD_ID]
            target_batch[i] = tgt[i] + [ac.EOS_ID] + (max_tgt_len - tgt_lens[i]) * [ac.PAD_ID]
    ...
    def get_iter(self, shuffle=False):
        if shuffle:
            self.args.logger.info('Shuffling data')
            if self.mode == ac.TRAIN and self.regen_batches:
                self.args.logger.info('Regenerate batches')
                self.args.logger.info('{} batches'.format(len(self.indices)))
                self.gen_batches()
            else:
